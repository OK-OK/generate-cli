 ...
 * @LastEditTime: 2022-06-23 17:43:07
...
module.exports = function getQuestions(type) {
  const questions = [
    ...
    },
    {
      ...
      name: "preset",
      choices: ({ type }) => {
        const list = ["Default（create default cli）"]
    ...
    ...packageQuestions,
    {
      ...
      name: "packages",
      choices: ({ type, viteType }) => {
        let list = packageList[type]
        // ts 版本默认引入element-plus
        if (type === "vite" && viteType === "ts") {
          list = list.filter(k => k.name !== "element-plus")
        }
        return list.concat(packageList["public"])
      }
    },
    {
      when: ({ preset }) => !preset.includes("default"),
